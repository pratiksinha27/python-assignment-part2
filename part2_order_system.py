...
for cat in categories:
    ...
    for item in menu:
        ...
        if details["category"] == cat:
            ...
            # convert True/False to readable form
            if details["available"] == True:
                status = "Available"
            else:
                status = "Unavailable"

            print(item.ljust(15), f"₹{details['price']:.2f}", "[" + status + "]")
...
print("Most expensive item:", max_item, f"₹{max_price:.2f}")

# Print items with price less than 150
print("\nItems under ₹150:")
for item in menu:
    if menu[item]["price"] < 150:
        print(item, f"₹{menu[item]['price']:.2f}")
...
# simulate customer order step by step
cart = []

# add item to cart and update quantity if already exists
...
# function to remove an item from cart by name
...
# function to update quantity of an existing item in cart
...
update_quantity("Paneer Tikka", 3)
print("Current cart:", cart)
...
for c in cart:
    ...
    print(c["item"].ljust(18), "x" + str(c["quantity"]), f"₹{total:.2f}")
...
print("----------------------------------")
print("Subtotal:".ljust(25), f"₹{subtotal:.2f}")
print("GST (5%):".ljust(25), f"₹{gst:.2f}")
print("Total Payable:".ljust(25), f"₹{total_pay:.2f}")
...
# create copy so original data stays safe
...
for c in cart:
    ...
    if inventory[item]["stock"] >= qty:
        ...
        print("Not enough stock for", item, "- only", available, "available")
...
for item in inventory:
    ...
    if stock <= level:
        print("⚠ Reorder Alert:", item, "— Only", stock, "unit(s) left (reorder level:", level, ")")
...
# printing all orders using enumerate
print("\nAll Orders:")
...
for date in sales_log:
    for o in sales_log[date]:
        all_orders.append((date, o))

# now use enumerate properly
